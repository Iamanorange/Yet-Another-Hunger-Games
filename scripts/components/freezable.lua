...
function Freezable:ResolveResistance()
    ...
    if TUNING.DISTPENALTY_REBALANCE and (self.inst.components.beard ~= nil) then
        resistance = resistance + self.inst.components.beard:GetInsulation()/7.5
    end
    return self.extraresist ~= nil
        and math.min(resistance * 2.5, resistance + self.extraresist)
        or resistance
end
