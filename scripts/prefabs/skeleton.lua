...
local function onworked(inst)
    ...
    local item = inst.components.inventory:FindItem(function(item) return true end)
    if item == nil then
        inst.components.workable:SetWorkLeft(0)
        return
    end
    inst.components.inventory:DropItem(item, true)
    if (inst.components.inventory:NumItems() > 0) and (inst.components.workable.workleft == 0) then
...
local function fnplayer()
    ...
    -- print("[Skeleton_fnplayer][SetWorkLeft] "..tostring(inst.components.inventory:NumItems()))
    -- print(inst.components.workable:GetDebugString())
